const Pub=require('./../Models/skilldev');
...
exports.getaddskilldev= (req,res) => {
    //res.cookie("user","Akshata");
    const user1=req.cookies['user'];
    if(user1) {
        res.status(200).render('addskilldev', {
            title: 'All Programs'
        });
...
exports.getviewskilldev= async (req,res) => {
    ...
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('viewskilldev', {
                categories: lcat1233,
                link: link123,
...
exports.getexportskilldev= async (req,res) => {
	...
	worksheet.columns = [
		...
		{ header: 'Type of Participation', key: 'engagementytpe', width: 30},
...
exports.geteditskilldev= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('editskilldev', {
                pub: leditcat,
                category: leditcat,
                title: 'Edit'
...
exports.createskilldev= async (req,res) => {
    ...
    try{
        ...
        if(user1) {
            const pub1= await Pub.create({
                ...
                date: req.body.date,
                noofstudenr: req.body.noofstudenr,
       ...
       res.status(200).render('addskilldev', {
        title: 'Add Skill Development Program'
    });
        // res.status(400).json({
        //     status:'Failed',
        //     message: err
...
exports.updateskilldev= async (req,res) => {
...
exports.deleteskilldev= async (req,res) => {
