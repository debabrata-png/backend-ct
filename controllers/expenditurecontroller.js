const Pub=require('./../Models/expenditure');
...
exports.getaddexpenditure= (req,res) => {
    //res.cookie("user","Akshata");
    const user1=req.cookies['user'];
    if(user1) {
        res.status(200).render('addexpenditure', {
...
exports.getviewexpenditure= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('viewexpenditure', {
                categories: lcat1233,
                link: link123,
...
exports.getexportexpenditure= async (req,res) => {
	...
	worksheet.columns = [
		...
		{ header: 'Infrastructure expenditure', key: 'infraexp', width: 30},
...
exports.geteditexpenditure= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('editexpenditure', {
                pub: leditcat,
                category: leditcat,
                title: 'Edit'
    ...
    try{
        ...
        if(user1) {
            const pub1= await Pub.create({
                ...
                infraexp: req.body.infraexp,
                totalexp: req.body.totalexp,
                academicexp: req.body.academicexp,
                physicalexp: req.body.physicalexp,
