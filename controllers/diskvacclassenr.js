    ...
    upload(req,res,function(err) {
        ...
        if(user1) {
            ...
            try {
                ...
                res.redirect('/viewvacclassenroll/' + req.params.id);
            } catch(err1) {
                req.flash("error", "Data not added. Invalid file format");
                res.redirect('/addvacclassenroll/' + req.params.id);
...
exports.getaddvacclassenrol= (req,res) => {
    //res.cookie("user","Akshata");
    const user1=req.cookies['user'];
    //console.log(user1);
    if(user1) {
        res.status(200).render('addvacclassenr', {
...
exports.getenroldash= async (req,res) => {
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('viewvacclassenr', {
                categories: lcat1233,
...
exports.updatevacclassenrol= async (req,res) => {
    ...
    try{
        ...
        if (type==1) {
            const lcat1= await Pat.findByIdAndUpdate( req.body.category_id,{
       ...
       res.redirect('/viewvacclassenroll/' + req.body.vacid);
    } catch(err) {
