...
exports.financeDashboard = async (req, res) => {
  ...
  const rows = invoices.map(inv => {
    ...
    return {
      invoiceid: inv._id,
      vendor: inv.poid?.vendorid?.vendorname || 'NA',
      poid: inv.poid?._id,
...
/* ================= VENDOR DRILLDOWN ================= */
exports.vendorInvoiceDrilldown = async (req, res) => {
  ...
  const result = invoices
    .filter(inv => inv.poid?.vendorid?.vendorname === vendor)
    .map(inv => {
      const paid = paymentMap[inv._id] || 0;
