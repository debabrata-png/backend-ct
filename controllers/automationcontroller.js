const Pub=require('./../Models/automation');
...
exports.getaddautomation= (req,res) => {
    //res.cookie("user","Akshata");
    const user1=req.cookies['user'];
    if(user1) {
        res.status(200).render('addautomation', {
...
exports.getviewautomation= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            .where('criteria')
            .equals('Exam Automation')
            .where('colid')
            .equals(colid);
            //res.status(200).send('Hello world for all the tours through db new router');
            res.status(200).render('viewautomation', {
                categories: lcat1233,
                link: link123,
                title: 'List of automation'
        });
        } else {
            req.flash("error", "You have been logged out. Please login to continue.");
            res.redirect('/login');
...
exports.geteditautomation= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('editautomation', {
                pub: leditcat,
                category: leditcat,
                title: 'Edit'
