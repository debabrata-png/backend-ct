const Pub=require('./../Models/rural');
...
exports.getaddrural= (req,res) => {
    //res.cookie("user","Akshata");
    const user1=req.cookies['user'];
    if(user1) {
        res.status(200).render('addrural', {
            title: 'All learners'
        });
...
exports.getviewrural= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            .where('criteria')
            .equals('Student seats against Rural learners Category')
            .where('colid')
            .equals(colid);
            //res.status(200).send('Hello world for all the tours through db new router');
            res.status(200).render('viewrural', {
                categories: lcat1233,
                link: link123,
...
exports.geteditrural= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('editrural', {
                pub: leditcat,
                category: leditcat,
                title: 'Edit'
...
exports.createrural= async (req,res) => {
...
exports.updaterural= async (req,res) => {
...
exports.deleterural= async (req,res) => {
