...
const Mjournal1 = require('../Models/mjournal1');  // Changed from accountfinds to mjournal1
...
// Mjournal1 (formerly accountfinds) CRUD Controllers
exports.dscreatemjournal1 = async (req, res) => {
  try {
    const {
      name,
      user,
    ...
    const newMjournal1 = new Mjournal1({
      name,
      user,
      colid,
      year,
    ...
    // res.status(500).json({
    //   success: false,
    //   message: 'Error creating mjournal1',
    //   error: error.message
    // });
  }
};

exports.dsgetmjournal1 = async (req, res) => {
  try {
    ...
    const mjournal1 = await Mjournal1.find(filter);
    return res.status(200).json({
      success: true,
      data: mjournal1
    });
    ...
    // res.status(500).json({
    //   success: false,
    //   message: 'Error fetching mjournal1',
    //   error: error.message
    // });
  }
};

exports.dsupdatemjournal1 = async (req, res) => {
  try {
    const { id, colid } = req.query;
    const updateData = req.body;
    if (!id || !colid) {
    ...
    const updated = await Mjournal1.findOneAndUpdate(
      { _id: id, colid: parseInt(colid) },
      updateData,
      { new: true }
    );
    ...
    if (!updated) {
      return res.status(404).json({
        success: false,
        message: 'Mjournal1 not found for this colid'
      });
    }
    ...
    // res.status(500).json({
    //   success: false,
    //   message: 'Error updating mjournal1',
    //   error: error.message
    // });
  }
};

exports.dsdeletemjournal1 = async (req, res) => {
  try {
    const { id, colid } = req.query;
    if (!id || !colid) {
      return res.status(400).json({
    ...
    const deleted = await Mjournal1.findOneAndDelete({
      _id: id,
      colid: parseInt(colid)
    });
    ...
    return res.status(200).json({
      success: true,
      message: 'Mjournal1 deleted successfully'
    });
  } catch (error) {
    // res.status(500).json({
    //   success: false,
    //   message: 'Error deleting mjournal1',
    //   error: error.message
    // });
  }
};
...
exports.dsmjournal1report = async (req, res) => {
  try {
    ...
    const data = await Mjournal1.find(filter);
    return res.status(200).json({
      success: true,
      data: data
    });
...
exports.dsgetyearsbycolid = async (req, res) => {
  try {
    ...
    const years = await Mjournal1.distinct('year', { colid: parseInt(colid) });
    return res.status(200).json({
      success: true,
