const Pat=require('./../Models/vacclass');
const Link=require('./../Models/uploadlink');
const Pat1=require('./../Models/vacattendance');


exports.getaddvacclass= (req,res) => {
    //res.cookie("user","Akshata");
    const user1=req.cookies['user'];
    if(user1) {
        res.status(200).render('addvacclass', {
            title: 'Add Class', 
            vacid: req.params.id
        });
...
exports.getjoinvacclass= (req,res) => {
    //res.cookie("user","Akshata");
    const user1=req.cookies['user'];
    if(user1) {
        res.status(200).render('joinvacclasscode', {
            title: 'Join Class',
            vacid: req.params.id
        });
...
exports.getviewvacclass= async (req,res) => {
    ...
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('viewvacclass', {
                categories: lcat1233,
                //link:link123,
                title: 'List class',
                vacid: req.params.id
...
exports.getviewvacclasstoday= async (req,res) => {
    //res.cookie("user","Akshata");
...
exports.getopenvacclassstud= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        today.setHours(today.getHours() - 1 + 5);
        today.setMinutes(today.getMinutes() + 30);
        tomorrow.setHours(tomorrow.getHours() + 24 + 5);
        tomorrow.setMinutes(tomorrow.getMinutes() + 30);   
        //console.log(today);
        ...
        if(user1) {
            ...
            .where('classdate')
            .lte(tomorrow);
            res.status(200).render('viewvacclassstud', {
                categories: lcat1233,
                //link:link123,
                title: 'List class'
...
exports.getclosedvacclassstud= async (req,res) => {
    //res.cookie("user","Akshata");
...
// exports.getallvacclassstud= async (req,res) => {
//     //res.cookie("user","Akshata");
...
exports.postjoinvacclass= async (req,res) => {
    //res.cookie("user","Akshata");
...
exports.getvacattend= async (req,res) => {
    ...
    try{
        ...
        if(user1) {
            ...
            if (cstatus==1) {
                const pat1= await Pat1.findOneAndUpdate({classid: req.params.id, regno: regno, colid: colid},{
                    name: name,
...
exports.getvacattreport= async (req,res) => {
    try{
        const user1=req.cookies['user'];
        ...
        if(user1) {
            const lcat1233= await Pat1.aggregate([
                { 
                    $match: {coursecode: req.params.id }
                },
                { 
                    $group: {
                        _id:['$regno','$name','$colid'], 
...
exports.getviewvacclassatt= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            .where('classid')
            .equals(req.params.id);
            res.status(200).render('viewvacclassattendance', {
                categories: lcat1233,
                //link:link123,
                title: 'List class participants'
...
exports.geteditvacclass= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('editvacclass', {
                pub: leditcat,
                title: 'Edit',
                vacid:req.query.classid
...
exports.createvacclass= async (req,res) => {
    ...
    try{
        ...
        if(user1) {
           ...
           res.redirect('/viewvacclass/' + req.params.id);
...
exports.createvacclassadv= async (req,res) => {
...
exports.updatevacclass= async (req,res) => {
...
exports.deletevacclass= async (req,res) => {
