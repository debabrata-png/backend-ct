const Pub=require('./../Models/querymode');
...
exports.getaddquerymode= (req,res) => {
    //res.cookie("user","Akshata");
    const user1=req.cookies['user'];
    if(user1) {
        res.status(200).render('addquerymode', {
            title: 'All modes'
        });
...
exports.getviewquerymode= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            .where('criteria')
            .equals('Types of Query Modes')
            .where('colid')
            .equals(colid);
            //res.status(200).send('Hello world for all the tours through db new router');
            res.status(200).render('viewquerymode', {
                categories: lcat1233,
                link: link123,
                title: 'List of modes'
        });
...
exports.geteditquerymode= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('editquerymode', {
                pub: leditcat,
                category: leditcat,
                title: 'Edit'
...
exports.createquerymode= async (req,res) => {
    ...
    try{
        ...
        if(user1) {
            ...
            year: req.body.year,
            type: req.body.fname,
            received: req.body.academicbody,
            resolved: req.body.department,
...
exports.updatequerymode= async (req,res) => {
...
exports.deletequerymode= async (req,res) => {
