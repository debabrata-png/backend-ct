...
exports.createRelativeGradingConfiguration = async (req, res) => {
  try {
    const payload = cleanPayload(req.body);
    const error = validatePayload(payload);
    if (error) return res.status(400).json({ success: false, message: error });
    const data = await RelativeGradingConfiguration.create(payload);
...
exports.getRelativeGradingConfigurations = async (req, res) => {
  try {
    const query = buildQuery(req.query);
    if (query.colid === undefined) return res.status(400).json({ success: false, message: "colid is required" });
    const data = await RelativeGradingConfiguration.find(query).sort({
      academicyear: 1,
      regulation: 1,
      programcode: 1,
      course: 1,
...
exports.updateRelativeGradingConfiguration = async (req, res) => {
  try {
    const payload = cleanPayload(req.body);
    const error = validatePayload(payload);
    if (error) return res.status(400).json({ success: false, message: error });
    const data = await RelativeGradingConfiguration.findOneAndUpdate(
...
exports.deleteRelativeGradingConfiguration = async (req, res) => {
  try {
    const data = await RelativeGradingConfiguration.findOneAndDelete({ _id: req.body.id, colid: toNumber(req.body.colid) });
    if (!data) return res.status(404).json({ success: false, message: "Record not found" });
...
exports.getRelativeGradingConfigurationOptions = async (req, res) => {
  try {
    ...
    const [courseMaps, configs] = await Promise.all([
      ...
      RelativeGradingConfiguration.find({ colid }).lean()
    ]);
