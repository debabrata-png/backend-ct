const Pub=require('./../Models/prisoner');
...
exports.getaddprisoner= (req,res) => {
    //res.cookie("user","Akshata");
    const user1=req.cookies['user'];
    if(user1) {
        res.status(200).render('addprisoner', {
            title: 'All learners'
        });
...
exports.getviewprisoner= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            .where('criteria')
            .equals('Student seats against Prisoners Category')
            .where('colid')
            .equals(colid);
            //res.status(200).send('Hello world for all the tours through db new router');
            res.status(200).render('viewprisoner', {
                categories: lcat1233,
                link: link123,
...
exports.geteditprisoner= async (req,res) => {
    //res.cookie("user","Akshata");
    ...
    try{
        ...
        if(user1) {
            ...
            res.status(200).render('editprisoner', {
                pub: leditcat,
                category: leditcat,
                title: 'Edit'
...
exports.createprisoner= async (req,res) => {
...
exports.updateprisoner= async (req,res) => {
...
exports.deleteprisoner= async (req,res) => {
